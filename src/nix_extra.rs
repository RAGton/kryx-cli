...
pub fn registry(args: &[String]) -> Result<(), String> {
    let bin = discover_real_bin("nix").ok_or_else(|| "nix not found in PATH".to_string())?;
...
pub fn nh_clean_all(args: &[String]) -> Result<(), String> {
    let bin = discover_real_bin("nh").ok_or_else(|| "nh not found in PATH".to_string())?;
...
mod tests {
    ...
    #[test]
    #[ignore = "requires nh in PATH; run with --ignored for full coverage"]
    fn nh_os_variant_accepts_known_variants() {
        // Requires nh in PATH; ignored by default to keep CI
        // dependency-free. Run with: cargo test -- --ignored
        for v in &["boot", "test", "dry-activate", "build"] {
            let r = nh_os_variant(v, &[]);
            // Only fail on validator-level errors; spawn failures
            // (e.g. nh missing) are accepted because the validator
            // is what we are actually testing here.
            if let Err(e) = r {
                assert!(
